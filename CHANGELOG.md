...
- Git 命令执行新增实时输出、进度条和停止按钮，主窗口与悬浮窗的长耗时命令可中途取消。

## [1.0.5] - 2026-05-22

### Added

- 悬浮窗快捷命令新增最近结果面板，支持查看和复制标准输出、错误输出及异常信息。
- 提交图谱新增关键词与作者筛选，支持快速定位提交、哈希、引用和作者。
- 提交图谱新增“加载更早提交”，避免固定数量截断导致早期历史不可见。
- 变更区新增文件级暂存、取消暂存和丢弃操作入口。
- 新增 Git 命令解析与风险识别的轻量测试。

### Changed

- 悬浮窗项目选择改用组件库下拉框，减少项目较多时的空间占用。
- 重构提交图谱 lane 渲染逻辑，统一节点、竖线和 merge 曲线的坐标系统，并让节点颜色跟随 lane 保持连贯。
- 缩小悬浮窗内部控件圆角，整体视觉更紧凑。
- 调整变更区布局，将提交信息输入框和提交按钮放到文件变更列表上方。
- 主窗口与悬浮窗快捷命令新增危险命令风险提示和显式确认，用于识别可能丢弃本地变更或改写远端历史的操作。
- Saved Commands 右键操作改为菜单，支持悬浮、取消悬浮、编辑和删除保存指令。
